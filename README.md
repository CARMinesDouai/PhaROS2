...
## what's happened
`ROS2` is a Middleware. So the node is broadcast on the network.
The informations send by the `joy_node` is send throught the network to PhaROS2 `turtlebot2_demo`. It's catch and treat by pharo. Once the `cmd_vel` message is create, the information go on the network one more to the `kobuki_node`. The turtlebot start moving.
